...
public class ElGamalAlice
{
	private static BigInteger computeY(BigInteger p, BigInteger g, BigInteger d)
	{
		...
		BigInteger y = g.modPow(d, p);   // computation of y = g^d mod p
	...
	private static BigInteger computeK(BigInteger p)
	{
		...
		// choosing a random number k using the cryptographically strong pseudo random number
		SecureRandom sRandom = new SecureRandom();
		int numBits = 1024;
		BigInteger k = new BigInteger(numBits, sRandom);
		
		//getting the value of p-1 
		BigInteger pMinusOne = p.subtract(BigInteger.ONE);
		
		// making sure that k and p-1 are relatively prime
		while(!k.gcd(pMinusOne).equals(BigInteger.ONE))
		{
			k = new BigInteger(numBits, sRandom);
		}
	...
	{
		// IMPLEMENT THIS FUNCTION;
		BigInteger a = g.modPow(k, p); // computation of: a = g^k mod p
